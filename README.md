...
Evaluation performed on `sample_questions_hybrid_eval.jsonl` (6 questions) with outputs in `outputs_hybrid.jsonl`:

| Metric | Value |
|--------|-------|
| **Coverage** | 6/6 questions answered (100%) |
| **Format Compliance** | 6/6 answers match format_hint (100%) |
| **SQL Generation Rate** | 5/6 questions used SQL (83.3%) |
| **Average Confidence** | 0.89 |
| **Confidence Range** | 0.60 - 0.95 |

**Breakdown:**
- **RAG-only questions:** 1/6 (16.7%) - Policy/document questions answered from retrieved chunks
- **SQL-enabled questions:** 5/6 (83.3%) - Questions requiring database queries
- **Format types handled:** int, float, object, list (all validated correctly)
- **Citations:** All answers include proper citations (doc chunks and/or database tables)
...
- `AGENT_PROFILE=1` (optional) to enable timing metrics.
...
- [ ] **README.md** - Contains:
  ...
  - [ ] Evaluation results with accuracy metrics (see "Evaluation Results" section)
...
# 2. Check trace log exists
python -c "import os; print('✓ Trace log found' if os.path.exists('outputs_hybrid_trace.jsonl') else '✗ Trace log missing')"

# 3. Verify all required files in agent/
python -c "import os; files = ['agent/graph_hybrid.py', 'agent/dspy_signatures.py', 'agent/rag/retrieval.py', 'agent/tools/sqlite_tool.py']; all_exist = all(os.path.exists(f) for f in files); print('✓ All agent files present' if all_exist else '✗ Some files missing')"
```
